...
chunks parquet ready for embedding (Phase 4).
...
Usage:
    uv run python -m voice_rag.chunking.build_chunks --languages hi ta --split validation
"""
...
def build_chunks_for_language(lang: str, split: str, config: ChunkingConfig | None = None) -> pd.DataFrame:
    ...
    if not passages_path.exists():
        raise FileNotFoundError(
            f"{passages_path} not found — run `voice_rag.ingestion.build_corpus` for "
