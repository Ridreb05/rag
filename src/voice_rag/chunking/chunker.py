"""Phase 3 — Chunking strategies A (fixed-token fallback), B (sentence-aware,
default), D (metadata-aware tagging), and the schema hook for E (hierarchical,
inert on this dataset). Strategy F (query-aware granularity) lives in
``chunking/granularity.py`` since it operates on queries, not passages.

Per the dataset analysis (docs/01-dataset-analysis.md): passages are already
short, pre-segmented units (median 34-60 words depending on language). This
chunker's default behavior for the overwhelming majority of passages is
therefore a no-op — return the passage as a single chunk — and only engages
sentence-aware or fixed-token splitting for the long tail that exceeds
...
from voice_rag.chunking.tokenizer import TokenCounter, whitespace_token_counter
...
class Chunk(BaseModel):
    ...
    # Strategy E hook (hierarchical): inert for MSMARCO-XI (no document/section
    # structure — see docs/01-dataset-analysis.md), populated once the system
    # is pointed at real long-form documents.
    level: str = "passage"
