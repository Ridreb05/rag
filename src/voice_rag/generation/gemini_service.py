"""Generative path via the Gemini API — currently the *working* generation
backend (the Claude integration in anthropic_service.py is code-verified
correct but blocked on Anthropic account credits; this is what's actually
live). Same interface as AnthropicGenerationService (a `generate(GenerationRequest)
-> GeneratedAnswer | None` method) so the harness can use either
interchangeably — see generation/harness.py.
...
class GeminiGenerationService:
    ...
    def _post_with_retries(self, payload: dict, trace_id: str) -> httpx.Response:
        ...
        'retries, ... error recovery'), matching what the Anthropic SDK
        gives Claude calls automatically (default max_retries=2) but which
        raw REST calls to Gemini need to implement explicitly."""
