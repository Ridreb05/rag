...
depends on which embedding model ends up indexing the text (BGE-M3 — an
XLM-RoBERTa-family SentencePiece tokenizer, a heavy transformers dependency
this module deliberately does not require). Dataset analysis also found
that whitespace word counts under-count information density for compounding
languages like Sanskrit, so the default here is explicitly an
approximation, not presented as exact.

Callers that need exact BGE-M3 token counts should pass a ``token_counter``
built from that model's tokenizer (see embeddings/service.py); the chunker
itself is written against the ``TokenCounter`` protocol below and does not
care which implementation it gets.
"""
...
def whitespace_token_counter(text: str) -> int:
    ...
    MSMARCO-XI passages (Sanskrit mean 42.9 words/passage vs.
