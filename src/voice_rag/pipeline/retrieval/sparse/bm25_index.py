"""BM25 sparse retrieval via Tantivy (embedded, no server).
...
class Bm25Index:
    ...
    def _get_searcher(self) -> tantivy.Searcher:
        """Returns a cached searcher, reloading the index only when needed.

        ``search`` used to call ``Index.reload()`` on every query. That
        re-reads the index metadata and reopens segment readers from disk
        each time — measured at ~11.5ms per query on the 964,603-chunk Hindi
        index (27.1ms with the reload vs 15.6ms without, identical results).
        A reload is only meaningful after a commit, so the searcher is cached
        here and explicitly invalidated by ``upsert_batch`` — index builds
        stay correct, and the read-only serving path stops paying for a
        freshness check that can never find anything new.
        """
    ...
    def upsert_batch(self, chunk_ids: list[str], texts: list[str]) -> None:
        ...
        writer.commit()
        self._index.reload()
        self._searcher = None  # force _get_searcher to pick up the new commit
