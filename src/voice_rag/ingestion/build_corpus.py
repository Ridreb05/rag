"""Phase 2 — Ingestion: raw MSMARCO-XI parquet -> normalized, deduplicated corpus.

Two tables are produced per (language, split):

- ``passages`` — one row per *unique* passage (deduplicated by a content
  hash of the normalized translated text, per Phase 1's finding that 1.4-3.6%
  of passage slots are exact duplicates and some passages are shared by 80+
  queries). This is what actually gets embedded/indexed in Phase 4 — indexing
  the raw exploded table would waste storage and skew retrieval towards
  over-represented passages.
...
Usage:
    uv run python -m voice_rag.ingestion.build_corpus --languages hi ta --split validation
"""
...
from voice_rag.ingestion.hf_source import download_split
from voice_rag.ingestion.schema import LANGUAGES
