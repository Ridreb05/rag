"""Phase 1 — Dataset analysis for ai4bharat/MSMARCO-XI.
...
Usage:
    uv run python -m voice_rag.ingestion.analyze --languages hi ta ur sa
    uv run python -m voice_rag.ingestion.analyze --languages all --footer-only
"""
...
from voice_rag.ingestion.hf_source import available_splits, download_split, read_footer_metadata
from voice_rag.ingestion.schema import LANGUAGES, LANGUAGES_WITHOUT_TRAIN

logger = logging.getLogger(__name__)

REPORTS_DIR = Path("reports/dataset_analysis")
DOCS_REPORT = Path("docs/dataset-analysis-report.md")
...
def write_markdown_summary(reports: list[dict[str, Any]], footer_only_langs: list[dict[str, Any]]) -> None:
    ...
    lines.append("")
    lines.append(
        "Generated by `voice_rag.ingestion.analyze`. Corpus-size figures come from "
    ...
    logger.info("Wrote %s", DOCS_REPORT)
