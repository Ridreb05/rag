"""Grounding / hallucination-detection validator (docs/03-harness-and-guardrails.md).
...
This is flagged as a follow-up risk in docs/09-roadmap-and-summary.md, not
silently assumed to work uniformly.
"""
...
class GroundingValidator:
    ...
    def validate_claim(self, claim_text: str, evidence_texts: list[str]) -> ClaimValidation:
        ...
        matches docs/03's "checked against its cited chunk(s)" (plural)."""
    ...
    def detect_contradiction(self, text_a: str, text_b: str) -> float:
        """Symmetric-ish contradiction check between two passages (docs/03's
        "conflicting sources" guardrail) — max of both premise/hypothesis orderings,
        since NLI contradiction is not perfectly symmetric in practice."""
